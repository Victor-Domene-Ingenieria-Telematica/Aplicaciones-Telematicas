...
const textosCarrito = {
    ...
    fr: { titulo: "COMMANDE", prod: "Produit", ptsUnit: "Points", cant: "Qté", ptsTotal: "Total Pts", accion: "Retirer", vaciar: "VIDER", granTotalPts: "TOTAL POINTS" },
    de: { titulo: "BESTELLUNG", prod: "Produkt", ptsUnit: "Punkte", cant: "Menge", ptsTotal: "Gesamt Pts", accion: "Löschen", vaciar: "BESTELLUNG LEEREN", granTotalPts: "GESAMTPUNKTE" }
};
...
function actualizarTablaPedidos() {
    ...
    if (zonaPedidos && tabla) {
        ...
        // Añadimos las nuevas columnas al encabezado
        tabla.innerHTML = `
            <tr class="cabecera-tabla">
                <th>${textosCarrito[idiomaActual].prod}</th>
                <th>${textosCarrito[idiomaActual].ptsUnit}</th>
                <th>${textosCarrito[idiomaActual].cant}</th>
                <th>${textosCarrito[idiomaActual].ptsTotal}</th>
        ...
        for (let clave in pedidos) {
            if(pedidos[clave] > 0) {
                ...
                // Sumamos a la hucha global
                totalPuntosGlobal = totalPuntosGlobal + puntosDeEstaLinea;
        ...
        if(hayPedidos) {
            ...
            // Actualizamos la tabla nueva de abajo con el resultado de las sumas
            if(etiquetaPuntos && valorPuntos) {
                etiquetaPuntos.textContent = textosCarrito[idiomaActual].granTotalPts;
                valorPuntos.textContent = totalPuntosGlobal;
            }
        } else {
