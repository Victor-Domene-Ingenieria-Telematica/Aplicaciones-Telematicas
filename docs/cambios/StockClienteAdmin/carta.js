...
// Control de Stock y Administrador
let stockProductos = {};
let modoAdmin = false;
...
const textosFiltros = {
    es: { ocultarAlc: "Ocultar alcohol", mostrarAlc: "Mostrar alcohol", verEsp: "Especialidades", verTodo: "Ver carta completa", modoAdmin: "Modo Administrador", modoCliente: "Modo Cliente", agotado: "AGOTADO" },
    en: { ocultarAlc: "Hide alcohol", mostrarAlc: "Show alcohol", verEsp: "Chef's Specials", verTodo: "Full Menu", modoAdmin: "Admin Mode", modoCliente: "Client Mode", agotado: "SOLD OUT" },
    fr: { ocultarAlc: "Masquer l'alcool", mostrarAlc: "Afficher l'alcool", verEsp: "Spécialités du Chef", verTodo: "Carte Complète", modoAdmin: "Mode Admin", modoCliente: "Mode Client", agotado: "ÉPUISÉ" },
    de: { ocultarAlc: "Alkohol ausblenden", mostrarAlc: "Alkohol anzeigen", verEsp: "Spezialitäten", verTodo: "Ganze Speisekarte", modoAdmin: "Admin-Modus", modoCliente: "Kundenmodus", agotado: "AUSVERKAUFT" }
};
...
function inicializarStock() {
    for (let sublista of lista_menu) {
        for (let i = 1; i < sublista.length; i++) {
            ...
            // Genera un stock aleatorio entre 1 y 4
            stockProductos[idProd] = Math.floor(Math.random() * 4) + 1;
        }
    }
}
...
function generarCarta() {
    ...
    for(let sublista of lista_menu) {
        ...
        for(let idProd of sublista.slice(1)) {
            ...
            // Logica de Stock
            let stockActual = stockProductos[idProd];
            let stockTag = document.createElement("div");
            stockTag.className = "etiqueta-stock";

            // Si no queda, lo marcamos
            if (stockActual === 0) {
                card.classList.add("agotado");
                stockTag.textContent = textosFiltros[idiomaActual].agotado;
                stockTag.style.color = "#dc3545"; // Rojo
            } else if (modoAdmin === true) {
                // Si queda y somos admin, vemos el número
                stockTag.textContent = "Stock: " + stockActual;
            }

            // El clic ahora vigila la estantería
            card.addEventListener("click", function() {
                if (stockProductos[idProd] > 0) {
                    pedidos[idProd]++;
                    stockProductos[idProd]--; // Quitamos 1 del stock
                    guardarPedidos(); 
                    
                    // Volvemos a generar la carta para que el stock se actualice
                    generarCarta(); 
                }
            });

            // Metemos la etiqueta del stock también en la tarjeta
            card.append(imgProd, nombre, stockTag);
            grid.append(card);
        }
...
if(btnVaciarObj) {
    btnVaciarObj.addEventListener("click", function() {
        for(let clave in pedidos) {
            stockProductos[clave] = stockProductos[clave] + pedidos[clave]; // Devolvemos TODO
            pedidos[clave] = 0;
        }
        guardarPedidos(); 
        generarCarta(); // Redibujamos para que desaparezcan todos los "AGOTADO"
    });
}
...
function actualizarTextosFiltros() {
    ...
    if(btnAlc && btnEsp && btnModo) {
        ...
        // Texto del botón Modo
        if (modoAdmin === true) {
            btnModo.textContent = textosFiltros[idiomaActual].modoCliente;
        } else {
            btnModo.textContent = textosFiltros[idiomaActual].modoAdmin;
        }
    }
}
...
let btnModoObj = document.querySelector("#btn-modo");
if (btnModoObj) {
    btnModoObj.addEventListener("click", function() {
        if (modoAdmin === true) {
            modoAdmin = false;
        } else {
            modoAdmin = true;
        ...
        generarCarta(); // Redibujamos para mostrar/ocultar los números
    });
}
