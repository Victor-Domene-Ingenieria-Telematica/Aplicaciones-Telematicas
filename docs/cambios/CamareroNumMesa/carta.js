...
// --- Traducciones para el modo camarero ---
const textosMesas = {
    es: { titulo: "SELECCIONE MESA", mesa: "MESA", finalizar: "FINALIZAR PEDIDO" },
    en: { titulo: "SELECT TABLE", mesa: "TABLE", finalizar: "FINISH ORDER" },
    fr: { titulo: "CHOISIR UNE TABLE", mesa: "TABLE", finalizar: "TERMINER LA COMMANDE" },
    de: { titulo: "TISCH AUSWÄHLEN", mesa: "TISCH", finalizar: "BESTELLUNG ABSCHLIESSEN" }
};
...
function generarTablaMesas() {
    const tablaMesas = document.querySelector("#tabla-lista-mesas");
    const tituloSeccionMesas = document.querySelector("#zona-mesas h2"); // Para el título "SELECCIONE MESA"

    if (tablaMesas) {
        // Ponemos el título de la sección en el idioma correcto
        if (tituloSeccionMesas) {
            tituloSeccionMesas.textContent = textosMesas[idiomaActual].titulo;
        }

        tablaMesas.innerHTML = ""; 
        
        for (let i = 1; i <= numMesasTotal; i++) {
            let tr = document.createElement("tr");
            let td = document.createElement("td");
            
            // Usamos el diccionario: "MESA", "TABLE", etc.
            td.textContent = textosMesas[idiomaActual].mesa + " " + i;
            td.className = "celda-mesa";
            ...
            tr.append(td);
            tablaMesas.append(tr);
        }
    }
}

function empezarPedido(numeroMesa) {
    mesaActual = numeroMesa;
    
    document.querySelector("#zona-mesas").classList.add("oculto");
    document.querySelector("#zona-trabajo").classList.remove("oculto");
    
    // Aquí también usamos el idioma para el indicador: "MESA 3", "TABLE 3"...
    document.querySelector("#indicador-mesa").textContent = textosMesas[idiomaActual].mesa + " " + numeroMesa;
...
// Evento para el botón verde de "Finalizar Pedido"
let btnFinalizarObj = document.querySelector("#btn-finalizar");
if (btnFinalizarObj) {
    btnFinalizarObj.addEventListener("click", function() {
        // Reseteamos la mesa a 0
        mesaActual = 0;
        
        // Ocultamos el carrito/menú y volvemos a la lista de mesas
        document.querySelector("#zona-trabajo").classList.add("oculto");
        document.querySelector("#zona-mesas").classList.remove("oculto");
    });
}
...
function actualizarTextosFiltros() {
    ...
    const btnFin = document.querySelector("#btn-finalizar"); // Capturamos el botón de finalizar

    if(btnAlc && btnEsp) {
        // (Tus ifs de alcohol y especialidades se quedan igual...)
        if(mostrandoAlcohol === true) btnAlc.textContent = textosFiltros[idiomaActual].ocultarAlc;
        else btnAlc.textContent = textosFiltros[idiomaActual].mostrarAlc;

        if(soloEspecialidades === true) btnEsp.textContent = textosFiltros[idiomaActual].verTodo;
        else btnEsp.textContent = textosFiltros[idiomaActual].verEsp;
    }

    // Traducimos el botón de Finalizar
    if (btnFin) {
        btnFin.textContent = textosMesas[idiomaActual].finalizar;
    }
}
...
function cambiarIdioma(nuevoIdioma) {
    ...
    // Volvemos a dibujar las mesas para que cambien su texto ("MESA" -> "TABLE")
    generarTablaMesas(); 

    // Si el camarero ya había elegido una mesa, traducimos el título grande de arriba
    if (mesaActual > 0) {
        document.querySelector("#indicador-mesa").textContent = textosMesas[idiomaActual].mesa + " " + mesaActual;
    }
}
