...
const calorias = {
    "b1": 0, "b2": 140, "b3": 130, "b4": 150, "b5": 120,
    "e1": 250, "e2": 600, "e3": 550, "e4": 400,
    "co1": 700, "co2": 950,
    "h1": 850, "h2": 900,
    "ca1": 1200, "ca2": 1300,
    "p1": 450, "p2": 500, "p3": 100
};
...
const textosFiltros = {
    es: { ocultarAlc: "Ocultar alcohol", mostrarAlc: "Mostrar alcohol", verEsp: "Especialidades", verTodo: "Ver carta completa", dietaOn: "Filtro Dieta: ON", dietaOff: "Filtro Dieta: OFF" },
    en: { ocultarAlc: "Hide alcohol", mostrarAlc: "Show alcohol", verEsp: "Chef's Specials", verTodo: "Full Menu", dietaOn: "Diet Filter: ON", dietaOff: "Diet Filter: OFF" },
    fr: { ocultarAlc: "Masquer l'alcool", mostrarAlc: "Afficher l'alcool", verEsp: "Spécialités du Chef", verTodo: "Carte Complète", dietaOn: "Filtre Régime: ON", dietaOff: "Filtre Régime: OFF" },
    de: { ocultarAlc: "Alkohol ausblenden", mostrarAlc: "Alkohol anzeigen", verEsp: "Spezialitäten", verTodo: "Ganze Speisekarte", dietaOn: "Diätfilter: ON", dietaOff: "Diätfilter: OFF" }
};
...
function generarCarta() {
    ...
    for(let sublista of lista_menu) {
        ...
        for(let idProd of sublista.slice(1)) {
            ...
            // Filtro Dieta (Ocultar si > 500)
            if(modoDieta === true && calorias[idProd] > 500)
                card.classList.add("oculto");
            ...
            // NUEVO: Etiqueta visual de Calorías
            let calTag = document.createElement("div");
            calTag.textContent = calorias[idProd] + " kcal";
            calTag.style.color = "#28a745"; 
            ...
            card.append(imgProd, nombre, calTag);
            grid.append(card);
        }
...
function actualizarTablaPedidos() {
    ...
    if (zonaPedidos && tabla) {
        ...
        let totalCaloriasGlobal = 0; // Hucha para las calorías

        for (let clave in pedidos) {
            if(pedidos[clave] > 0) {
                hayPedidos = true;

                // Calculamos las carorias
                let caloriasLinea = calorias[clave] * pedidos[clave];
                totalCaloriasGlobal = totalCaloriasGlobal + caloriasLinea;
        ...
        if(hayPedidos) {
            ...
            document.querySelector("#valor-calorias").textContent = totalCaloriasGlobal;
        } else {
...
function actualizarTextosFiltros() {
    ...
    const btnDieta = document.querySelector("#btn-dieta");
    ...
    if(btnAlc && btnEsp) {
        ...
        // Boton dieta
        if(modoDieta === true) {
            btnDieta.textContent = textosFiltros[idiomaActual].dietaOn;
        } else {
            btnDieta.textContent = textosFiltros[idiomaActual].dietaOff;
        }
...
function toggleFiltro(tipo) {
    ...
    for(let card of todosLosPlatos) {
        ...
        let imagen = card.querySelector(".img-producto");
        if (imagen) {
            let idDelPlato = imagen.id; // Sacamos el id ("b1", "e2"...)
            if (modoDieta === true && calorias[idDelPlato] > 500) {
                card.classList.add("oculto");
            }
        }
    }
...
let btnDietaObj = document.querySelector("#btn-dieta");
if(btnDietaObj) {
    btnDietaObj.addEventListener("click", function() { toggleFiltro('dieta'); });
}
