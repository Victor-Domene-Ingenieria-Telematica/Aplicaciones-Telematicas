    ...
    if (zonaPedidos && tabla) {
        ...
        for (let clave in pedidos) {
            if(pedidos[clave] > 0) {
                ...
                let costeLinea = preciosBase[clave] * pedidos[clave];
                subtotalGlobal = subtotalGlobal + costeLinea;
        ...
        if(hayPedidos) {
            ...
            // Matemáticas de la propina
            let propinaEnEuros = subtotalGlobal * (porcentajePropinaActual / 100);
            let totalFinal = subtotalGlobal + propinaEnEuros;

            // Mostrar en pantalla forzando 2 decimales
            document.querySelector("#valor-subtotal").textContent = subtotalGlobal.toFixed(2) + " €";
            document.querySelector("#valor-propina").textContent = propinaEnEuros.toFixed(2) + " €";
            document.querySelector("#valor-total-final").textContent = totalFinal.toFixed(2) + " €";
...
// Función que cambia el valor y manda recalcular
function cambiarPropina(nuevoPorcentaje) {
    porcentajePropinaActual = nuevoPorcentaje;
    actualizarTablaPedidos();
}

let btnPropina0 = document.querySelector("#btn-propina-0");
if (btnPropina0) {
    btnPropina0.addEventListener("click", function() {
        cambiarPropina(0);
    });
}

let btnPropina5 = document.querySelector("#btn-propina-5");
if (btnPropina5) {
    btnPropina5.addEventListener("click", function() {
        cambiarPropina(5);
    });
}

let btnPropina10 = document.querySelector("#btn-propina-10");
if (btnPropina10) {
    btnPropina10.addEventListener("click", function() {
        cambiarPropina(10);
    });
}
