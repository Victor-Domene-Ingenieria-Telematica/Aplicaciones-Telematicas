...
const PorcentajeReducido = 20; 
let modoReducido = false; 

// Precios base para cada producto
const preciosBase = {
    "b1": 2.00, "b2": 2.50, "b3": 2.50, "b4": 3.00, "b5": 3.50,
    "e1": 12.00, "e2": 8.50, "e3": 9.00, "e4": 11.00,
    "co1": 22.00, "co2": 45.00,
    "h1": 14.00, "h2": 15.50,
    "ca1": 18.00, "ca2": 19.50,
    "p1": 6.50, "p2": 6.00, "p3": 5.00
};
...
const textosFiltros = {
    es: { ocultarAlc: "Ocultar alcohol", mostrarAlc: "Mostrar alcohol", verEsp: "Especialidades", verTodo: "Ver carta completa", btnReducido: "Reducido", btnNormal: "Normal" },
    en: { ocultarAlc: "Hide alcohol", mostrarAlc: "Show alcohol", verEsp: "Chef's Specials", verTodo: "Full Menu", btnReducido: "Reduced", btnNormal: "Normal" },
    fr: { ocultarAlc: "Masquer l'alcool", mostrarAlc: "Afficher l'alcool", verEsp: "Spécialités du Chef", verTodo: "Carte Complète", btnReducido: "Réduit", btnNormal: "Normal" },
    de: { ocultarAlc: "Alkohol ausblenden", mostrarAlc: "Alkohol anzeigen", verEsp: "Spezialitäten", verTodo: "Ganze Speisekarte", btnReducido: "Reduziert", btnNormal: "Normal" }
};

let pedidos = {};

const textosCarrito = {
    es: { titulo: "PEDIDO", prod: "Producto", precioU: "Precio", cant: "Cant.", total: "Total", accion: "Quitar", vaciar: "VACIAR PEDIDO", granTotal: "TOTAL" },
    en: { titulo: "ORDER", prod: "Product", precioU: "Price", cant: "Qty", total: "Total", accion: "Remove", vaciar: "CLEAR ORDER", granTotal: "TOTAL" },
    fr: { titulo: "COMMANDE", prod: "Produit", precioU: "Prix", cant: "Qté", total: "Total", accion: "Retirer", vaciar: "VIDER", granTotal: "TOTAL" },
    de: { titulo: "BESTELLUNG", prod: "Produkt", precioU: "Preis", cant: "Menge", total: "Gesamt", accion: "Löschen", vaciar: "BESTELLUNG LEEREN", granTotal: "GESAMT" }
};
...
function obtenerPrecioFinal(idProd) {
    let precio = preciosBase[idProd];
    
    if (modoReducido === true) {
        // Aplicamos el descuento del 20%
        let descuento = precio * (PorcentajeReducido / 100);
...
function generarCarta() {
    ...
    for(let sublista of lista_menu) {
        ...
        for(let idProd of sublista.slice(1)) {
            ...
            // Etiqueta del precio en la carta
            let precioTag = document.createElement("div");
            precioTag.textContent = obtenerPrecioFinal(idProd) + " EUR";
            precioTag.style.color = "#17a2b8";
            ...
            // Añadimos el precio al final de la tarjeta
...
function actualizarTablaPedidos() {
    ...
    const etiquetaTotal = document.querySelector("#etiqueta-total");
    ...
    if (zonaPedidos && tabla) {
        ...
        if(etiquetaTotal)
            etiquetaTotal.textContent = textosCarrito[idiomaActual].granTotal;
        ...
        tabla.innerHTML = `
            <tr class="cabecera-tabla">
                ...
                <th>${textosCarrito[idiomaActual].precioU}</th>
                <th>${textosCarrito[idiomaActual].cant}</th>
                <th>${textosCarrito[idiomaActual].total}</th>
                <th>${textosCarrito[idiomaActual].accion}</th>
            </tr>
        `;

        let hayPedidos = false;
        let importeGlobal = 0; // Hucha para el importe total
        ...
        for (let clave in pedidos) {
            if(pedidos[clave] > 0) {
                ...
                // Calculo de precios
                let precioUnitarioStr = obtenerPrecioFinal(clave);
                let precioUnitarioNum = parseFloat(precioUnitarioStr);
                let cantidad = pedidos[clave];
                let precioLineaNum = precioUnitarioNum * cantidad;
                importeGlobal = importeGlobal + precioLineaNum; // Sumamos a la hucha
                ...
                let tdPrecioU = document.createElement("td");
                tdPrecioU.textContent = precioUnitarioStr + " €";
                
                let tdCant = document.createElement("td");
                tdCant.textContent = cantidad;
                tdCant.className = "cantidad-pedido text-center";

                let tdTotalLinea = document.createElement("td");
                tdTotalLinea.textContent = precioLineaNum.toFixed(2) + " €";
                ...
                tr.append(tdNombre, tdPrecioU, tdCant, tdTotalLinea, tdAccion);
                tabla.append(tr);
            }
        }
        ...
        if(hayPedidos) {
            ...
            if (valorTotal) {
                valorTotal.textContent = importeGlobal.toFixed(2) + " EUR";
            }
        } else {
...
function actualizarTextosFiltros() {
    ...
    if(btnAlc && btnEsp && btnPrecios) {
        ...
        // Texto del botón Precios
        if(modoReducido === true) {
            btnPrecios.textContent = textosFiltros[idiomaActual].btnNormal;
        } else {
            btnPrecios.textContent = textosFiltros[idiomaActual].btnReducido;
        }
    }
}
...
function toggleFiltro(tipo) {
    ...
    for(let card of todosLosPlatos) {
        ...
        // Por defecto, le quitamos la clase oculto
...
// Escuchador del botón de precios
let btnPreciosObj = document.querySelector("#btn-precios");
if(btnPreciosObj) {
    btnPreciosObj.addEventListener("click", function() {
        modoReducido = !modoReducido; // Invertimos el estado
        
        // Refrescamos la carta y el carrito para que se recalculen todos los precios
        generarCarta(); 
    });
}
