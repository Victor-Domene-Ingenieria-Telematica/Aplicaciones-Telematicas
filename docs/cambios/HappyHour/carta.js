...
const textosHappy = {
    es: { on: "Happy Hour: ON", off: "Happy Hour: OFF" },
    en: { on: "Happy Hour: ON", off: "Happy Hour: OFF" },
    fr: { on: "Happy Hour: ON", off: "Happy Hour: OFF" },
    de: { on: "Happy Hour: ON", off: "Happy Hour: OFF" }
};
    ...
    if (zonaPedidos && tabla) {
        ...
        for (let clave in pedidos) {
            if(pedidos[clave] > 0) {
                ...
                // Si la oferta está activa y el producto es una bebida (empieza por 'b')
                if (happyHourActiva === true && clave.charAt(0) === 'b') {
                    // Dividimos la cantidad entre 2 y nos quedamos con el número entero
                    // Ej: 2/2 = 1 gratis | 3/2 = 1 gratis | 4/2 = 2 gratis
                    unidadesGratis = Math.floor(cantidad / 2);
                }
                ...
                // Si le corresponden unidades gratis, lo mostramos al lado de la cantidad
                if (unidadesGratis > 0) {
                    tdCant.innerHTML = cantidad + ` <span style="color: #ffc107; font-size: 0.9rem;">(¡${unidadesGratis} GRATIS!)</span>`;
                } else {
...
function actualizarTextosFiltros() {
    ...
    if(btnAlc && btnEsp) {
        ...
        if (btnHappy) {
            if (happyHourActiva === true) {
                btnHappy.textContent = textosHappy[idiomaActual].on;
            } else {
                btnHappy.textContent = textosHappy[idiomaActual].off;
            }
        }  
    }
}
...
let btnHappyObj = document.querySelector("#btn-happy");
if (btnHappyObj) {
    btnHappyObj.addEventListener("click", function() {
        // Invertimos el estado (si era false pasa a true, y viceversa)
        happyHourActiva = !happyHourActiva;
        
        // Actualizamos el texto del botón
        actualizarTextosFiltros();
        
        // Recalculamos la tabla del pedido para que aparezcan o desaparezcan los mensajes de GRATIS
        actualizarTablaPedidos(); 
    });
}
